...
if {$USE_RM_BLOCK_NAME_AS_LABEL} {
	...
	copy_block -from ${CHIP_FINISH_BLOCK_NAME} -to ${ICV_IN_DESIGN_BLOCK_NAME}
	current_block ${ICV_IN_DESIGN_BLOCK_NAME}
}
link_block
...
redirect -tee -file ${REPORTS_DIR}/${ICV_IN_DESIGN_BLOCK_NAME}.report_app_options.start {report_app_options -non_default *}
redirect -file ${REPORTS_DIR}/${ICV_IN_DESIGN_BLOCK_NAME}.report_lib_cell_purpose.rpt {report_lib_cell -objects [get_lib_cells] -column {full_name:20 valid_purposes}}
...
if {[get_app_option_value -name signoff.check_drc.runset] != ""} {
	...
	set_app_options -name signoff.check_drc.run_dir -value $ICV_IN_DESIGN_DRC_CHECK_RUNDIR ;# RM default z_ADR_before
	signoff_check_drc -error_data $ICV_IN_DESIGN_DRC_CHECK_RUNDIR ;# RM default z_ADR_before
	...
	if {$ICV_IN_DESIGN_ADR} {
		set_app_options -name signoff.fix_drc.init_drc_error_db -value $ICV_IN_DESIGN_DRC_CHECK_RUNDIR ;# RM default z_ADR_before
		...
		if {$ICV_IN_DESIGN_ADR_DPT_RULES != ""} {
			...
			signoff_fix_drc -select_rules {$ICV_IN_DESIGN_DPT_RULES}
		...
		set_app_options -name signoff.check_drc.run_dir -value $ICV_IN_DESIGN_POST_ADR_RUNDIR ;# RM default z_ADR_after
		signoff_check_drc -error_data $ICV_IN_DESIGN_POST_ADR_RUNDIR ;# RM default z_ADR_after
		save_block
	}
...
if {$DESIGN_STYLE == "hier"} {
        ...
        if {$USE_RM_BLOCK_NAME_AS_LABEL} {
           ...
           derive_hier_antenna_property -design ${ICV_IN_DESIGN_BLOCK_NAME}
        }
}
save_lib
