...
# smkcow : VCK related settings here
...
redirect -file ${REPORTS_DIR}/${CLOCK_OPT_CTS_BLOCK_NAME}.report_lib_cell_purpose.rpt {report_lib_cell -objects [get_lib_cells] -column {full_name:20 valid_purposes}}
...
if {[file exists [which $TCL_REGULAR_MSCTS_FILE]] || [file exists [which $TCL_STRUCTURAL_MSCTS_FILE]]} { 
	if {$USE_RM_BLOCK_NAME_AS_LABEL} {
		...
		save_block -as ${CLOCK_OPT_CTS_BLOCK_NAME}_MSCTS
	}
}
...
if {$REPORT_QOR_REPORT_CONGESTION} {
	if {!$REPORT_QOR} {set REPORT_PREFIX $CLOCK_OPT_CTS_BLOCK_NAME}
